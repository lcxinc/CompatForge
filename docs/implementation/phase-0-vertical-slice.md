...
该切片只编译计划：不下载 Runtime、不启动 Wine、不创建 Bottle、不修改注册表。下一切片应先在 Mac-Win 建立 `RuntimeClient` 与 ABI smoke test，再把一次 legacy launch 的“计划阶段”切到 Rust；实际执行仍由 legacy runner 承担，直到 process supervisor 和 Wine Provider 完成。
